    ...
    internal class AbpNHibernateInterceptor : EmptyInterceptor
    {
        ...
        public override bool OnSave(object entity, object id, object[] state, string[] propertyNames, IType[] types)
        {
            ...
            if (entity is IEntity<Guid>)
            {
                ...
                if (guidEntity.IsTransient()) guidEntity.Id = _guidGenerator.Value.Create();
            }
        ...
        {
            if (entity.GetType().IsDefined(typeof(DisableDateTimeNormalizationAttribute), true)) return true;

            NormalizeDateTimePropertiesForEntity(entity, state, propertyNames, types);
            return true;
        }

        private static void NormalizeDateTimePropertiesForEntity(object entity, object[] state, string[] propertyNames,
            IList<IType> types)
        {
            for (var i = 0; i < types.Count; i++)
            {
                var prop = entity.GetType().GetProperty(propertyNames[i]);
                if (prop != null && prop.IsDefined(typeof(DisableDateTimeNormalizationAttribute), true)) continue;

                if (types[i].IsComponentType) NormalizeDateTimePropertiesForComponentType(state[i], types[i]);

                if (types[i].ReturnedClass != typeof(DateTime) && types[i].ReturnedClass != typeof(DateTime?)) continue;
        ...
        private static void NormalizeDateTimePropertiesForComponentType(object componentObject, IType type)
        {
            ...
            for (int i = 0; i < componentType.PropertyNames.Length; i++)
            {
                ...
                if (componentType.Subtypes[i].IsComponentType)
                {
                    ...
                    if (prop.IsDefined(typeof(DisableDateTimeNormalizationAttribute), true)) continue;

                    var value = prop.GetValue(componentObject, null);
                    NormalizeDateTimePropertiesForComponentType(value, componentType.Subtypes[i]);
                }

                if (componentType.Subtypes[i].ReturnedClass != typeof(DateTime) &&
                    componentType.Subtypes[i].ReturnedClass != typeof(DateTime?)) continue;

                var subProp = componentObject.GetType().GetProperty(propertyName);
                if (subProp == null) continue;

                if (subProp.IsDefined(typeof(DisableDateTimeNormalizationAttribute), true)) continue;
