...
public class AbpJsonActionResultWrapper : IAbpActionResultWrapper
{
    public void Wrap(FilterContext context)
    {
        ...
        if (jsonResult == null) throw new ArgumentException("Action Result should be JsonResult!");

        if (!(jsonResult.Value is AjaxResponseBase)) jsonResult.Value = new AjaxResponse(jsonResult.Value);
    }
}