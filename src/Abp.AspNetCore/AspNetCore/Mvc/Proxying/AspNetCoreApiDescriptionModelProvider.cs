...
public class AspNetCoreApiDescriptionModelProvider : IApiDescriptionModelProvider, ISingletonDependency
{
    ...
    private void AddApiDescriptionToModel(ApiDescription apiDescription, ApplicationApiDescriptionModel model)
    {
        ...
        {
            Logger.Warn(
                $"Controller '{controllerModel.Name}' contains more than one action with name '{methodName}' for module '{moduleModel.Name}'. Ignored: " +
    ...
    {
        if (!_apiProxyScriptingConfiguration.RemoveAsyncPostfixOnProxyGeneration) return method.Name;

        if (!method.IsAsync()) return method.Name;

        var normalizedName = method.Name.RemovePostFix("Async");
        if (controllerModel.Actions.ContainsKey(normalizedName)) return method.Name;
    ...
    {
        if (!apiDescription.ParameterDescriptions.Any()) return;
    ...
    public string GetMethodParamName(ParameterInfo parameterInfo)
    {
        ...
        if (modelNameProvider == null) return parameterInfo.Name;
    ...
    private string GetModuleName(ApiDescription apiDescription)
    {
        ...
        if (controllerType == null) return AbpControllerAssemblySetting.DefaultServiceModuleName;

        foreach (var controllerSetting in _configuration.ControllerAssemblySettings.Where(setting =>
                     setting.TypePredicate(controllerType)))
