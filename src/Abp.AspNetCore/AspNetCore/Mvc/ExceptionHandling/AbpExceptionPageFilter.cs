...
public class AbpExceptionPageFilter : IAsyncPageFilter, ITransientDependency
{
        ...
        PageHandlerExecutionDelegate next)
    {
        ...
        if (pageHandlerExecutedContext.Exception == null) return;
        ...
        if (wrapResultAttribute.LogError) LogHelper.LogException(Logger, pageHandlerExecutedContext.Exception);
    ...
    {
        if (!ActionResultHelper.IsObjectResult(context.HandlerMethod.MethodInfo.ReturnType)) return;
        ...
        if (!wrapResultAttribute.WrapOnError) return;
