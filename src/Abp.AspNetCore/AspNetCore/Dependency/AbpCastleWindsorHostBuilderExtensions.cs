...
public static class AbpCastleWindsorHostBuilderExtensions
{
    ...
    /// <returns></returns>
    public static IHostBuilder UseCastleWindsor(this IHostBuilder hostBuilder,
        [NotNull] IWindsorContainer windsorContainer)
    {
        Check.NotNull(windsorContainer, nameof(windsorContainer));

        return hostBuilder
            .ConfigureServices(services => { services.AddSingleton(windsorContainer); })
