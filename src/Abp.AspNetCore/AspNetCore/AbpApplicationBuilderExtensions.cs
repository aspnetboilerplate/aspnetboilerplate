...
{
    private const string AuthorizationExceptionHandlingMiddlewareMarker =
        "_AbpAuthorizationExceptionHandlingMiddleware_Added";
    ...
    public static void UseAbp([NotNull] this IApplicationBuilder app,
        Action<AbpApplicationBuilderOptions> optionsAction)
    {
        ...
        if (options.UseCastleLoggerFactory) app.UseCastleLoggerFactory();
        ...
        if (options.UseSecurityHeaders) app.UseAbpSecurityHeaders();
    }
    ...
    public static void UseCastleLoggerFactory(this IApplicationBuilder app)
    {
        ...
        if (castleLoggerFactory == null) return;
        ...
        Action<RequestLocalizationOptions> optionsAction = null)
    {
        ...
        using (var languageManager = iocResolver.ResolveAsDisposable<ILanguageManager>())
        {
            ...
            if (iocResolver.IsRegistered<ILogger<RequestLocalizationOptions>>())
                ...
                {
                    logger.Object.LogInformation(
                        $"Supported Request Localization Cultures: {string.Join(",", supportedCultures.Select(c => c))}");
                }
            ...
            userProvider.CookieProvider =
                options.RequestCultureProviders.OfType<CookieRequestCultureProvider>().FirstOrDefault();
            userProvider.HeaderProvider = options.RequestCultureProviders
                .OfType<AbpLocalizationHeaderRequestCultureProvider>().FirstOrDefault();
    ...
    {
        if (app.Properties.ContainsKey(AuthorizationExceptionHandlingMiddlewareMarker)) return app;
