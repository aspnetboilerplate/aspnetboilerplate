    ...
    where TUser : AbpUser<TUser>
{
    ...
    {
        if (!AutoSaveChanges || _unitOfWorkManager.Current == null) return Task.CompletedTask;
    ...
    {
        if (!AutoSaveChanges || _unitOfWorkManager.Current == null) return;
    ...
    /// The <see cref="Task"/> that represents the asynchronous operation, containing the user matching the specified <paramref name="userId"/> if it exists.
    /// </returns>
    public virtual TUser FindById(string userId, CancellationToken cancellationToken = default)
    {
        return _unitOfWorkManager.WithUnitOfWork(() =>
        {
            cancellationToken.ThrowIfCancellationRequested();
        ...
        CancellationToken cancellationToken = default)
    {
        _unitOfWorkManager.WithUnitOfWork(() =>
        {
            ...
            if (IsInRole(user, normalizedRoleName, cancellationToken)) return;
        ...
        CancellationToken cancellationToken = default)
    {
        await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            ...
            foreach (var claim in claims) user.Claims.Add(new UserClaim(user, claim));
        });
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!user.LockoutEndDateUtc.HasValue) return Task.FromResult<DateTimeOffset?>(null);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!user.LockoutEndDateUtc.HasValue) return (DateTimeOffset?)null;
    ...
    {
        _unitOfWorkManager.WithUnitOfWork(() => { _userPermissionSettingRepository.Delete(s => s.UserId == user.Id); });
    }
