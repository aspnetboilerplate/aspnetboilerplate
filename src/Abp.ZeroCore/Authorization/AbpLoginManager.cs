    ...
    where TUser : AbpUser<TUser>
{
    ...
    protected virtual async Task<TTenant> GetDefaultTenantAsync()
    {
        var tenant = await TenantRepository.FirstOrDefaultAsync(
            t => t.TenancyName == AbpTenantBase.DefaultTenantName
        );
        if (tenant == null) throw new AbpException("There should be a 'Default' tenant if multi-tenancy is disabled!");
    ...
    {
        var tenant = TenantRepository.FirstOrDefault(t => t.TenancyName == AbpTenantBase.DefaultTenantName);
        if (tenant == null) throw new AbpException("There should be a 'Default' tenant if multi-tenancy is disabled!");
