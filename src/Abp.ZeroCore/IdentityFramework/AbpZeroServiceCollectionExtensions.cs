...
public static class AbpZeroServiceCollectionExtensions
{
    ...
    {
        return services.AddAbpIdentity<TTenant, TUser, TRole>(null);
    }
        ...
        where TUser : AbpUser<TUser>
    {
        ...
        services.TryAddScoped(typeof(UserClaimsPrincipalFactory<TUser, TRole>),
            provider => provider.GetService(typeof(AbpUserClaimsPrincipalFactory<TUser, TRole>)));
        ...
        services.TryAddScoped(typeof(SecurityStampValidator<TUser>),
            provider => provider.GetService(typeof(AbpSecurityStampValidator<TTenant, TRole, TUser>)));
        services.TryAddScoped(typeof(ISecurityStampValidator),
            provider => provider.GetService(typeof(AbpSecurityStampValidator<TTenant, TRole, TUser>)));
        ...
        services.TryAddScoped(typeof(IUserStore<TUser>),
            provider => provider.GetService(typeof(AbpUserStore<TRole, TUser>)));

        //AbpRoleStore
        services.TryAddScoped<AbpRoleStore<TRole, TUser>>();
        services.TryAddScoped(typeof(IRoleStore<TRole>),
            provider => provider.GetService(typeof(AbpRoleStore<TRole, TUser>)));
        ...
        services.TryAddScoped(typeof(IFeatureValueStore),
            provider => provider.GetService(typeof(AbpFeatureValueStore<TTenant, TUser>)));
