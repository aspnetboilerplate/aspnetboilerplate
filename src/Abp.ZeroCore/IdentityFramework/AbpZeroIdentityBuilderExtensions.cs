...
public static class AbpZeroIdentityBuilderExtensions
{
    ...
    {
        var abpManagerType =
            typeof(AbpSignInManager<,,>).MakeGenericType(builder.TenantType, builder.RoleType, builder.UserType);
        ...
        where TLogInManager : class
    {
        ...
        var abpManagerType =
            typeof(AbpLogInManager<,,>).MakeGenericType(builder.TenantType, builder.RoleType, builder.UserType);
        builder.Services.AddScoped(type, provider => provider.GetService(abpManagerType));
        builder.Services.AddScoped(abpManagerType, type);
        return builder;
    }

    public static AbpIdentityBuilder AddAbpUserClaimsPrincipalFactory<TUserClaimsPrincipalFactory>(
        this AbpIdentityBuilder builder)
        where TUserClaimsPrincipalFactory : class
    {
        ...
        builder.Services.AddScoped(
            typeof(AbpUserClaimsPrincipalFactory<,>).MakeGenericType(builder.UserType, builder.RoleType),
            services => services.GetRequiredService(type));
    ...
    public static AbpIdentityBuilder AddAbpSecurityStampValidator<TSecurityStampValidator>(
        this AbpIdentityBuilder builder)
        where TSecurityStampValidator : class, ISecurityStampValidator
    {
        var type = typeof(TSecurityStampValidator);
        builder.Services.AddScoped(typeof(SecurityStampValidator<>).MakeGenericType(builder.UserType),
            services => services.GetRequiredService(type));
        builder.Services.AddScoped(
            typeof(AbpSecurityStampValidator<,,>).MakeGenericType(builder.TenantType, builder.RoleType,
                builder.UserType), services => services.GetRequiredService(type));
