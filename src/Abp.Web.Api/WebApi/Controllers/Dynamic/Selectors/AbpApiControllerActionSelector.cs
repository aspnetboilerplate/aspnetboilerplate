    ...
    public class AbpApiControllerActionSelector : ApiControllerActionSelector
    {
        ...
        public override HttpActionDescriptor SelectAction(HttpControllerContext controllerContext)
        {
            ...
            if (!controllerContext.ControllerDescriptor.Properties.TryGetValue("__AbpDynamicApiControllerInfo",
                    out controllerInfoObj)) return GetDefaultActionDescriptor(controllerContext);
            ...
            if (!hasActionName) return GetActionDescriptorByCurrentHttpVerb(controllerContext, controllerInfo);

            //Get action name from route
            var serviceNameWithAction = controllerContext.RouteData.Values["serviceNameWithAction"] as string;
            if (serviceNameWithAction == null) return GetDefaultActionDescriptor(controllerContext);
        ...
        private HttpActionDescriptor GetActionDescriptorByCurrentHttpVerb(HttpControllerContext controllerContext,
            DynamicApiControllerInfo controllerInfo)
        {
            ...
            return new DynamicHttpActionDescriptor(_configuration, controllerContext.ControllerDescriptor,
                actionsByVerb[0]);
        }

        private HttpActionDescriptor GetActionDescriptorByActionName(HttpControllerContext controllerContext,
