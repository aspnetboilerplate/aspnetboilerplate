    ...
    internal class TypeScriptDefinitionProxyGenerator : ITransientDependency
    {
        ...
        public string Generate(DynamicApiControllerInfo controllerInfo, string servicePrefix)
        {
            ...
            foreach (var methodInfo in _controllerInfo.Actions.Values)
            {
                ...
                if (returnType == "void")
                    script.AppendLine(string.Format("            {0} ({1}): abp.IPromise; ",
                        methodInfo.ActionName.ToCamelCase(), GetMethodInputParameter(methodInfo.Method)));
                else
                    script.AppendLine(string.Format("            {0} ({1}): abp.IGenericPromise<{2}>; ",
                        methodInfo.ActionName.ToCamelCase(), GetMethodInputParameter(methodInfo.Method), returnType));
            }
