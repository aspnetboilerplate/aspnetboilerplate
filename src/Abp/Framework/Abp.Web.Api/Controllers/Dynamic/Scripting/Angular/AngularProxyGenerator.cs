    ...
    internal class AngularProxyGenerator : IScriptProxyGenerator
    {
        ...
        public string Generate()
        {
            ...
            foreach (var methodInfo in _controllerInfo.Actions.Values)
            {
                ...
                script.AppendLine("                        url: '" + PlainActionUrl(methodInfo) + "',");

                CreateActionScriptWriter(_controllerInfo, methodInfo).WriteTo(script);
        ...
        protected string PlainActionUrl(DynamicApiActionInfo methodInfo)
        {
            return "/api/services/" + _controllerInfo.ServiceName + "/" + methodInfo.ActionName;
        }
