...
namespace Abp.Runtime.Caching.Memory
{
    ...
    public class AsyncThreadSafeObjectCache<TValue> where TValue : class
    {
        ...
        public AsyncThreadSafeObjectCache(ObjectCache cache)
            : this(
                cache,
                new CacheItemPolicy {SlidingExpiration = TimeSpan.FromMinutes(DefaultSlidingCacheDurationAsMinutes)})
        {
        }
        ...
        /// <param name="factoryMethod">A factory method to create item if it's not exists in cache</param>
        public async Task<TValue> GetAsync(string key, DateTimeOffset absoluteExpiration,
            Func<Task<TValue>> factoryMethod)
        {
            return
                await GetAsync(key, () => new CacheItemPolicy {AbsoluteExpiration = absoluteExpiration}, factoryMethod);
        }
        ...
        /// <param name="factoryMethod">A factory method to create item if it's not exists in cache</param>
        public async Task<TValue> GetAsync(string key, Func<CacheItemPolicy> cacheItemPolicy,
            Func<Task<TValue>> factoryMethod)
        {
            var cacheKey = key;
