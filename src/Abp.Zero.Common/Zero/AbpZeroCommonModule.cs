...
namespace Abp.Zero
{
    ...
    public class AbpZeroCommonModule : AbpModule
    {
        ...
        {
            IocManager
                .RegisterIfNot<IAbpZeroEntityTypes,
                    AbpZeroEntityTypes>(); //Registered on services.AddAbpIdentity() for Abp.ZeroCore.
        ...
        public override void Initialize()
        {
            ...
            IocManager.Register<IMultiTenantLocalizationDictionary, MultiTenantLocalizationDictionary>(
        ...
        {
            if (typeof(IAbpZeroFeatureValueStore).IsAssignableFrom(handler.ComponentModel.Implementation) &&
                !IocManager.IsRegistered<IAbpZeroFeatureValueStore>())
                IocManager.IocContainer.Register(
                    Component.For<IAbpZeroFeatureValueStore>().ImplementedBy(handler.ComponentModel.Implementation)
                        .Named("AbpZeroFeatureValueStore").LifestyleTransient()
                );
        }
        ...
        private void FillMissingEntityTypes()
        {
            using (var entityTypes = IocManager.ResolveAsDisposable<IAbpZeroEntityTypes>())
            {
                ...
                using (var typeFinder = IocManager.ResolveAsDisposable<ITypeFinder>())
                {
                    ...
                    entityTypes.Object.Tenant = types.FirstOrDefault(t =>
                        typeof(AbpTenantBase).IsAssignableFrom(t) && !t.GetTypeInfo().IsAbstract);
                    entityTypes.Object.Role = types.FirstOrDefault(t =>
                        typeof(AbpRoleBase).IsAssignableFrom(t) && !t.GetTypeInfo().IsAbstract);
                    entityTypes.Object.User = types.FirstOrDefault(t =>
                        typeof(AbpUserBase).IsAssignableFrom(t) && !t.GetTypeInfo().IsAbstract);
                }
            }
        }
        ...
        {
            if (IocManager.IsRegistered<ITenantCache>()) return;
