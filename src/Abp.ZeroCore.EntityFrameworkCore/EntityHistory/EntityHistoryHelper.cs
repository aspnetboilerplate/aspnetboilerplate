...
public class EntityHistoryHelper : EntityHistoryHelperBase, IEntityHistoryHelper, ITransientDependency
{
    ...
    public virtual EntityChangeSet CreateEntityChangeSet(ICollection<EntityEntry> entityEntries)
    {
        ...
        if (!IsEntityHistoryEnabled) return changeSet;
        ...
        foreach (var entityEntry in entityEntries)
        {
            ...
            if (shouldTrackEntity.HasValue && !shouldTrackEntity.Value) continue;

            if (!IsTypeOfEntity(typeOfEntity) && !entityEntry.Metadata.IsOwned()) continue;

            var shouldAuditEntity = IsTypeOfAuditedEntity(typeOfEntity);
            if (shouldAuditEntity.HasValue && !shouldAuditEntity.Value) continue;
            ...
            if (!shouldAuditEntity.HasValue && entityEntry.Metadata.IsOwned())
            {
                ...
                if (shouldAuditOwnerEntity.HasValue && !shouldAuditOwnerEntity.Value) continue;
                ...
                if (shouldAuditOwnerProperty.HasValue && !shouldAuditOwnerProperty.Value) continue;
            }
    ...
    public virtual async Task SaveAsync(EntityChangeSet changeSet)
    {
        ...
        if (changeSet.EntityChanges.Count == 0) return;
        ...
        foreach (var entityChange in changeSet.EntityChanges)
        {
            ...
            foreach (var property in foreignKey.Properties)
            {
                var shouldSaveProperty =
                    property.PropertyInfo == null // Shadow properties or if mapped directly to a field
                        ? null
                        : IsAuditedPropertyInfo(entityEntryType, property.PropertyInfo);

                if (shouldSaveProperty.HasValue && !shouldSaveProperty.Value) continue;
            ...
            foreach (var propertyChange in entityChange.PropertyChanges)
            {
                ...
                var isForeignKeyShadowProperty = property.IsShadowProperty() &&
                                                 foreignKeys.Any(fk =>
                                                     fk.Properties.Any(p => p.Name == propertyChange.PropertyName));
            ...
            foreach (var propertyChange in propertyChangesToRemove) entityChange.PropertyChanges.Remove(propertyChange);

            if (!isAuditedEntity && entityChange.PropertyChanges.Count == 0) entityChangesToRemove.Add(entityChange);
        }

        foreach (var entityChange in entityChangesToRemove) changeSet.EntityChanges.Remove(entityChange);
    }
