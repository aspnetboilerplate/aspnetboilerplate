...
namespace Abp.EntityFramework.Utils
{
    internal static class DateTimePropertyInfoHelper
    {
        ...
        {
            if (entityType.IsDefined(typeof(DisableDateTimeNormalizationAttribute), true)) return;
            ...
            dateTimePropertyInfos.DateTimePropertyInfos.ForEach(property =>
            {
                ...
                if (dateTime.HasValue) property.SetValue(entity, Clock.Normalize(dateTime.Value));
            });
        ...
        {
            if (!complexProperty.PropertyType.IsDefined(typeof(ComplexTypeAttribute), true)) return;
