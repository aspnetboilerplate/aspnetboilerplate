    ...
    where TDbContext : DbContext
{
    ...
    private static readonly ConcurrentDictionary<Type, bool> EntityIsDbQuery = new();
    ...
    {
        if (TransactionProvider == null) return null;
    ...
    public override TPrimaryKey InsertAndGetId(TEntity entity)
    {
        ...
        if (MayHaveTemporaryKey(entity) || entity.IsTransient()) GetContext().SaveChanges();
    ...
    {
        if (typeof(TPrimaryKey) == typeof(byte)) return true;

        if (typeof(TPrimaryKey) == typeof(int)) return Convert.ToInt32(entity.Id) <= 0;

        if (typeof(TPrimaryKey) == typeof(long)) return Convert.ToInt64(entity.Id) <= 0;
