    ...
    public class FeaturesScriptManager : IFeaturesScriptManager, ITransientDependency
    {
        ...
        public async Task<string> GetScriptAsync()
        {
            ...
            for (var i = 0; i < allFeatures.Count; i++)
            {
                ...
                script.AppendLine("        '" + HttpEncode.JavaScriptStringEncode(feature.Name.Replace("'", @"\'")) + "': {");
                script.AppendLine("             value: '" + HttpEncode.JavaScriptStringEncode(currentValues[feature.Name].Replace(@"\", @"\\").Replace("'", @"\'")) + "'");
                script.Append("        }");
