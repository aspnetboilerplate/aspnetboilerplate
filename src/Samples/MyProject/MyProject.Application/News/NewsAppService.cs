...
namespace MyProject.News
{
    public class NewsAppService : ApplicationService, INewsAppService
    {
        ...
        public GetNewsOutput GetNews(GetNewsInput input)
        {
            //Called specific GetAllWithPeople method of task repository.
            var news = _newsRepository.GetAllList(x=>x.Title.Contains(input.Title));

            //Used AutoMapper to automatically convert List<Task> to List<TaskDto>.
            return new GetNewsOutput
            {
                News = Mapper.Map<List<NewsDto>>(news)
            };
        }
        ...
        public News CreateNews(CreateNewsInput input)
        {
            //We can use Logger, it's defined in ApplicationService class.
            Logger.Info("Creating a news for input: " + input);
