...
namespace Abp.Tests.Configuration
{
    public class SettingManager_Tests : TestBaseWithLocalIocManager
    {
        ...
        private class MemorySettingStore : ISettingStore
        {
            ...
            public MemorySettingStore()
            {
                _settings = new List<SettingInfo>
                {
                    ...
                    new SettingInfo(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"), null, MyAllLevelsSetting,
                        "tenant 1 stored value"),
                    new SettingInfo(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                        Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"), MyAllLevelsSetting, "user 1 stored value"),
                    new SettingInfo(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                        Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000002"), MyAllLevelsSetting, "user 2 stored value"),
            ...
            public Task<SettingInfo> GetSettingOrNullAsync(Guid? tenantId, Guid? userId, string name)
            {
                return
                    Task.FromResult(
                        _settings.FirstOrDefault(s => s.TenantId == tenantId && s.UserId == userId && s.Name == name));
            }
        ...
        public async Task Should_Change_Setting_Values()
        {
            ...
            await
                settingManager.ChangeSettingForTenantAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    MyAllLevelsSetting, "tenant 1 changed value");
            ...
            await
                settingManager.ChangeSettingForUserAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    MyAllLevelsSetting, "user 1 changed value");
        ...
        public async Task Should_Delete_Setting_Values_On_Default_Value()
        {
            ...
            (await
                store.GetSettingOrNullAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"), MyAllLevelsSetting)).ShouldNotBe(null);
            ...
            await
                settingManager.ChangeSettingForUserAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    MyAllLevelsSetting, "tenant 1 stored value");
            (await
                store.GetSettingOrNullAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"), MyAllLevelsSetting)).ShouldBe(null);

            //We can get tenant's setting value
            (await
                store.GetSettingOrNullAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"), null, MyAllLevelsSetting))
                .ShouldNotBe(null);
            ...
            await
                settingManager.ChangeSettingForTenantAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    MyAllLevelsSetting, "application level stored value");
            (await
                store.GetSettingOrNullAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                    Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"), MyAllLevelsSetting)).ShouldBe(null);
        ...
        public async Task Should_Get_All_Values()
        {
            ...
            (await settingManager.GetAllSettingValuesForTenantAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001")))
                .Count.ShouldBe(1);
            (await settingManager.GetAllSettingValuesForTenantAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000002")))
                .Count.ShouldBe(0);
            (await settingManager.GetAllSettingValuesForTenantAsync(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000003")))
                .Count.ShouldBe(0);

            (await
                settingManager.GetAllSettingValuesForUserAsync(
                    new UserIdentifier(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                        Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001")))).Count.ShouldBe(1);
            (await
                settingManager.GetAllSettingValuesForUserAsync(
                    new UserIdentifier(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                        Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000002")))).Count.ShouldBe(1);
            (await
                settingManager.GetAllSettingValuesForUserAsync(
                    new UserIdentifier(Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000001"),
                        Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000003")))).Count.ShouldBe(0);
        }
        ...
        public async Task Should_Get_Correct_Values()
        {
            ...
            (await settingManager.GetSettingValueAsync(MyAllLevelsSetting)).ShouldBe("tenant 1 stored value");
                //Because no user value in the store

            session.TenantId = Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000003");
            session.UserId = Guid.Parse("FFFFFFFF-FFFF-FFFF-FFFF-000000000003");
            (await settingManager.GetSettingValueAsync(MyAllLevelsSetting)).ShouldBe("application level stored value");
                //Because no user and tenant value in the store
            ...
            (await settingManager.GetSettingValueForApplicationAsync(MyNotInheritedSetting)).ShouldBe(
                "application value");
            (await settingManager.GetSettingValueForTenantAsync(MyNotInheritedSetting, session.TenantId.Value)).ShouldBe
                ("default-value");
