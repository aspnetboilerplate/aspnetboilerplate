    ...
    public class MemoryCacheManager_Tests : TestBaseWithLocalIocManager
    {
        public MemoryCacheManager_Tests()
        {
            ...
            LocalIocManager.Resolve<ICachingConfiguration>()
                .ConfigureAll(cache => { cache.DefaultSlidingExpireTime = defaultSlidingExpireTime; });
        ...
        {
            LocalIocManager.Using<MyClientPropertyInjects>(
                client => {
        ...
        public void Simple_Get_Set_Test()
        {
            ...
            _cache.Get("B", () => new MyCacheItem {Value = 44}).Value.ShouldBe(43);
                //Does not call factory, so value is not changed
        }
    }
}