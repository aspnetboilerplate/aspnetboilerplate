...
namespace Abp.Web.MultiTenancy
{
    public class DomainTenantResolveContributor : ITenantResolveContributor, ITransientDependency
    {
        ...
        {
            if (_multiTenancyConfiguration.DomainFormat.IsNullOrEmpty()) return null;
            ...
            if (!result.IsMatch || !result.Matches.Any()) return null;

            var tenancyName = result.Matches[0].Value;
            if (tenancyName.IsNullOrEmpty()) return null;

            if (string.Equals(tenancyName, "www", StringComparison.OrdinalIgnoreCase)) return null;
