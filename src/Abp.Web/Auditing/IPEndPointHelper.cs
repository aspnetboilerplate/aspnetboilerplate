    ...
    internal class IPEndPointHelper
    {
        ...
        internal static bool TryParse(ReadOnlySpan<char> s, out IPEndPoint result)
        {
            ...
            if (lastColonPos > 0)
            {
                ...
                else if (s.Slice(0, lastColonPos).LastIndexOf(':') == -1) addressLength = lastColonPos;
            }
            ...
            if (IPAddress.TryParse(s.Slice(0, addressLength).ToString(), out IPAddress address))
            {
                ...
                if (addressLength == s.Length ||
                    uint.TryParse(s.Slice(addressLength + 1).ToString(), NumberStyles.None,
                        CultureInfo.InvariantCulture, out port) && port <= IPEndPoint.MaxPort)
        ...
        {
            if (TryParse(s, out IPEndPoint result)) return result;
