...
public class FeatureValueStoreTests : AbpZeroTestBase
{
    ...
    public void GetTenantFeatureCacheItem_ShouldEnableFilterMayHaveTenant_Test()
    {
        ...
        {
            _tenantFeatureRepository.Insert(new TenantFeatureSetting(tenant.Id, AppFeatures.SimpleBooleanFeature,
                "true"));
    ...
    public async Task GetTenantFeatureCacheItemAsync_ShouldEnableFilterMayHaveTenant_Test()
    {
        ...
        using (var uow = _unitOfWorkManager.Begin())
        {
            await _tenantFeatureRepository.InsertAsync(new TenantFeatureSetting(tenant.Id,
                AppFeatures.SimpleBooleanFeature, "true"));
            await _unitOfWorkManager.Current.SaveChangesAsync();

            // Assert (before disable filter)
            (await _featureValueStore.GetValueOrNullAsync(tenant.Id, AppFeatures.SimpleBooleanFeature))
                .ShouldBe("true");
            ...
            using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
            {
                ...
                (await _featureValueStore.GetValueOrNullAsync(tenant.Id, AppFeatures.SimpleBooleanFeature))
                    .ShouldBe("true");
                (await _featureValueStore.GetValueOrNullAsync(tenant2.Id, AppFeatures.SimpleBooleanFeature))
                    .ShouldBeNull();
            }
