...
public class UserManager_Permission_Tests : AbpZeroTestBase
{
    ...
    public async Task Should_Check_IsGranted_Correctly_When_Logged_In_As_Host_Then_Switched_To_Tenant()
    {
        ...
        var user = UsingDbContext(defaultTenantId,
            context =>
            {
                return context.Users.Single(f =>
                    f.TenantId == defaultTenantId && f.UserName == AbpUserBase.AdminUserName);
            });
        ...
        using (var uow = _unitOfWorkManager.Begin())
        {
            ...
            {
                isGranted = await _permissionChecker.IsGrantedAsync(user.ToUserIdentifier(),
                    AppPermissions.TestPermission);
                isGranted.ShouldBe(true);
            }
        }
    }
    ...
    public async Task Should_Inherit_Permission_From_Role_Of_Organization_Unit()
    {
        ...
        // Arrange
        var defaultTenantId = 1;
        var organizationUnit = UsingDbContext(defaultTenantId,
            context =>
            {
                return context.OrganizationUnits.Single(ou =>
                    ou.TenantId == defaultTenantId && ou.DisplayName == "OU1");
            });
        ...
        //Actual
        var user = UsingDbContext(defaultTenantId,
            context => { return context.Users.Single(u => u.TenantId == defaultTenantId && u.UserName == "user1"); });
