...
public class AbpPerRequestRedisCache_Test : PerRequestRedisCacheTestsBase<AbpPerRequestRedisCacheTestModule>
{
    ...
    public AbpPerRequestRedisCache_Test()
    {
        LocalIocManager.Resolve<ICachingConfiguration>().Configure("MyTestCacheItems",
            cache => { cache.DefaultSlidingExpireTime = TimeSpan.FromHours(12); });
        LocalIocManager.Resolve<ICachingConfiguration>().Configure("MyPerRequestRedisTestCacheItems",
            cache => { cache.DefaultSlidingExpireTime = TimeSpan.FromHours(24); });

        _perRequestRedisCache = LocalIocManager.Resolve<IAbpPerRequestRedisCacheManager>()
            .GetCache<string, MyCacheItem>("MyPerRequestRedisTestCacheItems");
    ...
    public void Should_Not_Change_Normal_Redis_Cache()
    {
        ...
        RedisDatabase.Received(2)
            .StringGet(Arg
                .Any<RedisKey>()); //redis cache tries to get value two times if value not exists see AbpCacheBase<TKey, TValue>.Get(TKey key, Func<TKey, TValue> factory)
    ...
    public void Should_Work_With_Null_Contexts()
    {
        ...
        var item1 = _perRequestRedisCache.Get(cacheKey, GetCacheValue);
        RedisDatabase.Received(3)
            .StringGet(Arg.Any<RedisKey>()); //since _currentHttpContext is null it should go to the redisdb again

        var item2 = _perRequestRedisCache.Get(cacheKey,
            GetCacheValue); //since _currentHttpContext is null it should go to the redisdb again
        RedisDatabase.Received(4).StringGet(Arg.Any<RedisKey>());
    ...
    {
        var item = await _perRequestRedisCache.GetAsync(cacheKey,
            () => Task.FromResult(new MyCacheItem { Value = cacheValue }));
    ...
    public async Task Should_Work_With_Null_Contexts_Async()
    {
        ...
        var item1 = await _perRequestRedisCache.GetAsync(cacheKey, GetCacheValue);
        await RedisDatabase.Received(3)
            .StringGetAsync(Arg.Any<RedisKey>()); //since _currentHttpContext is null it should go to the redisdb again

        var item2 = await _perRequestRedisCache.GetAsync(cacheKey,
            GetCacheValue); //since _currentHttpContext is null it should go to the redisdb again
