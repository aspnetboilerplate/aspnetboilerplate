    ...
    private void CreateInitialData()
    {
        UsingDbContext(
            context =>
            {
                ...
                {
                    DeletionTime =
                        DateTime.SpecifyKind(DateTime.Parse("2019-01-01T00:00:00"), DateTimeKind.Unspecified),
                    BlogTime = new BlogTime()
                    {
                        LastAccessTime = DateTime.SpecifyKind(DateTime.Parse("2019-02-01T00:00:00"),
                            DateTimeKind.Unspecified),
                        LatestPosTime = DateTime.SpecifyKind(DateTime.Parse("2019-03-01T00:00:00"),
                            DateTimeKind.Unspecified)
                    }
                };
        ...
        using (var context = LocalIocManager.Resolve<SupportDbContext>())
        {
            context.Tickets.AddRange(
                ...
                new Ticket
                {
                    EmailAddress = "david@aspnetboilerplate.com", Message = "an inactive message", IsActive = false,
                    TenantId = 1
                },
                new Ticket
                {
                    EmailAddress = "smith@aspnetboilerplate.com", Message = "an active message of tenant 42",
                    TenantId = 42
                }
            );
