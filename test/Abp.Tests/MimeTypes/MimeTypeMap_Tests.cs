...
namespace Abp.Tests.MimeTypes
{
    public class MimeTypeMap_Tests
    {
        ...
        public static IEnumerable<object[]> Check_If_It_Contains_All_Mappings_Data()
        {
            var mappings = BuildMappings();
            foreach (var mapping in mappings)
            {
                yield return new object[] {mapping.Key, mapping.Value};
            }
        }

        [Theory]
        [MemberData(nameof(Check_If_It_Contains_All_Mappings_Data))]
        public void Check_If_It_Contains_All_Mappings(string key, string expected)
        {
            if (key.StartsWith("."))
            {
            ...
            {
                _mimeTypeMap.GetExtension(key).ShouldBe(expected);
            }
        }
