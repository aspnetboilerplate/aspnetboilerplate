...
namespace Abp.TestBase.SampleApplication.Tests.EntityFramework.GraphDiff
{
    public class GraphDiffExtensions_Tests : SampleApplicationTestBase
    {
        ...
        [Fact]
        public void
            Disattached_Entity_Can_Be_Attached_To_Update_Corresponding_Existing_Entity_With_Navigation_Properties()
        {
            var unitOfWorkManager = Resolve<IUnitOfWorkManager>();
            using (var unitOfWork = unitOfWorkManager.Begin())
            {
                ...
                johnDoeAfterBeeingAttached.Id.ShouldBe(johnDoeFromContext
                    .Id); //As entity was detached (but not deleted), it should be updated (not re-created)
                johnDoeAfterBeeingAttached.Name
                    .ShouldBe(
                        "John Doe Junior"); //As entity was detached (but not deleted), it should be updated (not re-created)
                johnDoeAfterBeeingAttached.ContactListId
                    .ShouldBe(list2.Id); //Entity should be attached with it's navigation property
